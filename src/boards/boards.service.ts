...
export class BoardService{
    ...
    async shareBoard(boardId: string, userEmail: string) {
      ...
      try {
        const sharedBoard = await this.prisma.boardMember.create({
          data: {
            boardId: boardId,
            userEmail: userEmail, 
          },
        ...
        return {
          message: 'Board successfully shared with the user',
          data: sharedBoard,
        };
      } catch (error) {
        throw new HttpException(
          'Error sharing the board with the user',
          HttpStatus.INTERNAL_SERVER_ERROR,
        );
      }
    }

    async starBoard(boardId: string, starBoardDto: StarBoardDto) {
      const { email } = starBoardDto;
    ...
    async unstarBoard(boardId: string, unstarBoardDto: UnstarBoardDto) {
      const { email } = unstarBoardDto;
