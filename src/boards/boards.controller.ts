...
import { ShareBoardDto } from "./dto/shared-board-dto";
import { UnstarBoardDto } from "./dto/unstar-board";
import { StarBoardDto } from "./dto/star-board.dto";
...
export class BoardController{
  ...
  @Post(':boardId/share')
  @ApiOperation({ summary: 'Share a board with a user' })
  @ApiResponse({ status: 200, description: 'Board successfully shared' })
  @ApiResponse({ status: 404, description: 'Board or User not found' })
  async shareBoard(
    @Param('boardId') boardId: string,
    @Body() shareBoardDto: ShareBoardDto,
  ) {
    return await this.boardService.shareBoard(boardId, shareBoardDto.userEmail);
  }
  ...
  async star(
    @Param('boardId') boardId: string,
    @Body() starBoardDto: StarBoardDto,
  ) {
    return await this.boardService.starBoard(boardId, starBoardDto);
  }
  ...
  async unstar(
    @Param('boardId') boardId: string,
    @Body() unstarBoardDto: UnstarBoardDto,
  ) {
    return await this.boardService.unstarBoard(boardId, unstarBoardDto);
  }
}